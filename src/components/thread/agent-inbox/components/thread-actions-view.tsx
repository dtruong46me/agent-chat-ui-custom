    ...
    <div className="flex min-h-full w-full flex-col gap-4">
      ...
      <div className="flex w-full flex-wrap items-center justify-between gap-2">
        <div className="flex items-center justify-start gap-2">
           {/* Giảm size chữ title */}
          <p className="text-lg font-semibold tracking-tight text-pretty">{threadTitle}</p>
          {threadId && <ThreadIdCopyable threadId={threadId} />}
        </div>
        <div className="flex flex-row items-center justify-start gap-1"> {/* Giảm gap */}
          {apiUrl && (
            <Button
              size="sm" // Giữ size sm
              variant="outline"
              className="flex items-center gap-1" // Bỏ bg-white
              onClick={handleOpenInStudio}
            >
              Studio
            </Button>
       ...
       {/* Giảm khoảng cách cho các nút Resolve/Ignore */}
      <div className="flex w-full flex-row items-center justify-start gap-2 mt-2">
        <Button
          size="sm" // Giảm size nút
          variant="outline"
          className="border-gray-500 font-normal text-gray-800" // Bỏ bg-white
          onClick={handleResolve}
          disabled={actionsDisabled}
        >
          Mark as Resolved
